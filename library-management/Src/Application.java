...
public class Application {
	...
	private void initialize() {
		...
		lblNewLabel.setBounds(71, 13, 657, 61);
		...
		btnSEARCH.setBounds(71, 248, 299, 25);
		Library.add(btnSEARCH);
		...
		btnCHECK_IN.setBounds(71, 123, 97, 25);
		Library.add(btnCHECK_IN);
		...
		btnCHECK_OUT.setBounds(71, 182, 101, 25);
		Library.add(btnCHECK_OUT);
		...
		btnBorrower.setBounds(245, 123, 125, 25);
		...
		btnFINES.setBounds(245, 182, 125, 25);
				...
				try{
							...
							String book_id,book_title,author_name, branch_id,branch_name,no_of_copies,available_copies="";
						            ...
						            q="SELECT B.isbn,B.TITLE,B.BRANCH_ID,B.branch_name,fullname,no_of_copies" +
						                        ...
						                        " where B.isbn like '%"+id+"%'" +
						                        " Group by  B.branch_id, B.isbn;";
						                
						            }
						            
						            else if(title.isEmpty() && !author.isEmpty() && id.isEmpty()){
						            	  q="SELECT B.isbn,B.TITLE,B.BRANCH_ID,B.branch_name,fullname,no_of_copies " +
						                        " FROM book_loans L  RIGHT JOIN " +
						                        "(select c.branch_id , c.no_of_copies, lb.branch_name,bk.isbn, bk.title, au.fullname " +
						                        " from " +
						        ...
						        q="SELECT B.isbn,B.TITLE,B.BRANCH_ID,B.branch_name,fullname,no_of_copies" +
						                        ...
						                        " on c.branch_id = lb.branch_id )) as B " +
						                        " ON (L.isbn=B.isbn AND L.BRANCH_ID=B.BRANCH_ID) " +
						                        " where B.title like '%"+title+"%' Group by  B.branch_id, B.isbn;";
						            }
						            else  if(!title.isEmpty() && !author.isEmpty() && id.isEmpty() ){
						            	  q = "SELECT B.isbn,B.TITLE,B.BRANCH_ID,B.branch_name,fullname,no_of_copies" +
						                        " FROM book_loans L  RIGHT JOIN " +
						                        ...
						                        " on c.branch_id = lb.branch_id )) as B " +
						                        " ON (L.isbn=B.isbn AND L.BRANCH_ID=B.BRANCH_ID) " +
						                        " where B.title like '%"+title+"%' AND B.fullname like '%"+author+"%' Group by  B.branch_id, B.isbn;";
						            }
						            else  if(!title.isEmpty() && author.isEmpty() && !id.isEmpty()){
						            	 q="SELECT B.isbn,B.TITLE,B.BRANCH_ID,B.branch_name,fullname,no_of_copies" +
						                        " FROM book_loans L  RIGHT JOIN " +
						                        ...
						                        " on c.branch_id = lb.branch_id )) as B " +
						                        " ON (L.isbn=B.isbn AND L.BRANCH_ID=B.BRANCH_ID) " +
						                        " where B.title like '%"+title+"%'AND B.isbn like '%"+id+"%' Group by  B.branch_id, B.isbn;";
						            }
						            else  if(title.isEmpty() && !author.isEmpty() && !id.isEmpty()){
						             q="SELECT B.isbn,B.TITLE,B.BRANCH_ID,B.branch_name,fullname,no_of_copies" +
						                        " FROM book_loans L  RIGHT JOIN " +
						                        ...
						                        " on c.branch_id = lb.branch_id )) as B " +
						                        " ON (L.isbn=B.isbn AND L.BRANCH_ID=B.BRANCH_ID) " +
						                        " where B.isbn like '%"+id+"%'and B.fullname like '%"+author+"%' Group by  B.branch_id, B.isbn;";
						            }
							             ...
							             q="SELECT B.isbn,B.TITLE,B.BRANCH_ID,B.branch_name,fullname,no_of_copies" +
							                        ...
							                        " on c.branch_id = lb.branch_id )) as B " +
							                        " ON (L.isbn=B.isbn AND L.BRANCH_ID=B.BRANCH_ID) " +
							                        " where B.isbn like '%"+id+"%'and B.fullname like '%"+author+"%' and B.title like '%"+title+"%' Group by  B.branch_id, B.isbn;";
							            }
						            else{
